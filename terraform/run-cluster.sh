...
if [ -f "${HOME}/.terraform.tfvars" ]; then
    OPTIONS="-var-file ${HOME}/.terraform.tfvars"
...
# process static terraform files if not already done
check_static=$(aws ec2 describe-instance-connect-endpoints | grep "create-complete")
if [ -z "${check_static}" ]; then
    ...
    echo "Skipping static terraform apply: already done"
fi
